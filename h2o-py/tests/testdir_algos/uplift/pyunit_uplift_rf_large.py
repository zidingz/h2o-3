...
def uplift_random_forest_smoke():
    ...
    assert 0.008 < uplift_predict_kl.mean().as_data_frame()["uplift_predict"][0] < 0.009, \
        "Not expected output: Mean uplift is suspiciously different. " \
        + str(uplift_predict_kl.mean().as_data_frame()["uplift_predict"][0])

    assert -0.0095 < uplift_predict_euc.mean().as_data_frame()["uplift_predict"][0] < -0.0085, \
        "Not expected output: Mean uplift is suspiciously different." \
        + str(uplift_predict_euc.mean().as_data_frame()["uplift_predict"][0].mean())

    assert 0.0075 < uplift_predict_chi.mean().as_data_frame()["uplift_predict"][0] < 0.0085, \
        "Not expected output: Mean uplift is suspiciously different." \
